...
def gmaps_image(location, zoom=19, size=[300,300]):
    ...
    image_url = "https://maps.googleapis.com/maps/api/staticmap?center=" + location + "&zoom=" + str(zoom) + "&size=" + str(size[0]) + "x" + str(size[1]) + "&maptype=satellite&key=" + os.environ.get('GOOGLE_API_KEY')
...
def gmaps_area(location, zoom=19, size=[300,300]): 
    location = location.replace(" ", "+")
    response = requests.get("https://maps.googleapis.com/maps/api/geocode/json?address=" + location + "&key=" + os.environ.get('GOOGLE_API_KEY'))
